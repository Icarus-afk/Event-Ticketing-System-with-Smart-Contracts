import { bytesToHex, bytesToUnprefixedHex, concatBytes, equalsBytes, importPublic, isValidPrivate, isValidPublic, privateToAddress, privateToPublic, publicToAddress, randomBytes, toChecksumAddress, unprefixedHexToBytes, utf8ToBytes, } from '@ethereumjs/util';
...
export class Wallet {
    ...
    static async fromV1(input, password) {
        ...
        const seed = await aes.decrypt(ciphertext, keccak256(derivedKey.subarray(0, 16)).subarray(0, 16), unprefixedHexToBytes(json.Crypto.IV), 'aes-128-cbc');
    ...
    static async fromV3(input, password, nonStrict = false) {
        ...
        const seed = await aes.decrypt(ciphertext, derivedKey.subarray(0, 16), unprefixedHexToBytes(json.crypto.cipherparams.iv), json.crypto.cipher);
        return new Wallet(seed);
    }
    /*
    ...
    static async fromEthSale(input, password) {
        ...
        const seed = await aes.decrypt(encseed.subarray(16), derivedKey, encseed.subarray(0, 16), 'aes-128-cbc', true);
