...
contract Ticket {
    ...
    modifier validEvent(uint256 _eventId) {
        require(
            _eventId < eventManagement.getTotalEvents(),
            "Invalid event ID"
        );
        _;
    }
    ...
    function issueTicket(
        address _to,
        uint256 _eventId
    ) public validEvent(_eventId) {
        require(
            !tickets[_to][_eventId],
            "Ticket already issued to this address for the event"
        );
        require(
            eventManagement.ticketsSold(_eventId, _to) <
                eventManagement.getTotalTickets(_eventId),
    ...
    function getTicketDetails(
        address _owner,
        uint256 _eventId
    ) public view returns (bool hasTicket) {
